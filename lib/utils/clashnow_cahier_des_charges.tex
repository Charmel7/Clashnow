...
\begin{itemize}
  \item \textbf{Équipe GEP (Montage/Brico)} : responsable de la partie physique. Découpe/adaptation des boîtiers, installation des boutons et LEDs, câblage des buzzers (fils + résistances). Connexions sur breadboards ou borniers selon le schéma. Vérification de la continuité et de la fixation des composants.
  \item \textbf{Équipe GMM (Codage/Interface)} : responsable du développement logiciel. Programmation de l’ESP32 (ex. via Arduino) pour lire les 10 entrées digitales et transmettre les signaux au PC via USB. Adaptation de l’application ClashNow pour recevoir les signaux. Tests de communication série et validation fonctionnelle.
\end{itemize}
